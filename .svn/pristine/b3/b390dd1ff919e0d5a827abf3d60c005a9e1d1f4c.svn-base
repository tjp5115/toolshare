...
class UserProfile(models.Model):
    ...
    def update(user, request):        
        ...
        newZone, create2 = ShareZone.objects.get_or_create(zipCode=request.POST['shareZone'])
        ...
        if group != newGroup:
            # Remove any tools from their sheds
            for tool in Tool.objects.all().filter(ownerID = user.id):
                print("hit it")
                tool.removeFromShed()
                tool.save()
